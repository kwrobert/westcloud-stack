...
def run(length: float):
    ...
    # Simudo imports inside this function because the client that is submitting
    # the code may not have these modules installed, and importing them at the
    # top of the file tries to import them as soon as this file is parsed
    import glob
    import json
    import logging
    import os
    ...
    import dolfin
    # TODO: Deal with share directory access permissions from worker processes
    os.chdir("/share/ubuntu")

    setup_dolfin_parameters()
    TypicalLoggingSetup(filename_prefix="kyle_test").setup()
...
"""
    ...
    # Get all the stuff in locals thats json serializable and write it to the
    # share. Also return it to the client 
    # locals_to_dump = {} 
    ...
    #         continue
    # with open(f"length_{length}_locals.json", "w+") as f:
    #     json.dump(locals_to_dump, f)
    # return locals_to_dump
    return True
...
def main():
    ...
    lengths_to_simulate = [0.4, 0.5, 0.6]
    print("Number of connected workers = ", num_of_workers)
    print(f"Lengths to simulate: {lengths_to_simulate}")
    futures = client.map(run, lengths_to_simulate)
